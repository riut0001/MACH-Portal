<template>
  <div class="form-overview">
    ...
    <div class="anon-form-submit-footer">
      If you have any questions please contact the support under <a :href="'mailto: '+ form.metadata.supportEmail">{{form.metadata.supportEmail}}</a> 
    </div>
  </div>
  ...
  methods: {
    ...
    getSubmission() {
      axios({
        method: 'post',
        url: 'https://www-3.mach.kit.edu/api/getFormSubmissions.php',
        data: {formId: this.formId, mode: 'select', anonSubmissionKey: this.anonSubmissionKey}
      }).then(response => {
        console.log(response.data)
